...
while nextToken != None:
    if nextToken == "firstStart":
      ...
      response=requests.get(f"{url}&continuationToken={nextToken}")
      result=response.json()
...
#            os.system(f"curl -v -k --username admin:admin123 --uplaod-file {file_name} {upload_url}")
#            os.system(f"curl -v -k -u admin:admin123 --uplaod-file {file_name} {upload_url}")
            os.system(f"curl -v -k -u admin:admin123 -T {file_name} {upload_url}")
